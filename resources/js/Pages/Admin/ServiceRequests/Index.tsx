...
export default function ServiceRequestsIndex({ requests, filters }: Props) {
  ...
  const applyFilter = (status: string) => {
    ...
    router.get('/admin/service-requests', status !== 'all' ? { status } : {}, { preserveState: true, replace: true })
  }

  const handleStatusChange = (id: number, status: string) => {
    router.put(`/admin/service-requests/${id}`, { status }, { preserveState: true })
  }

  const handleDelete = (id: number) => {
    if (!confirm('Are you sure you want to delete this request?')) return
    router.delete(`/admin/service-requests/${id}`)
  }
  ...
  return (
    <AdminLayout>
      <div className="mx-auto max-w-5xl">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[#073B33]">Service Requests</h1>
        </div>
              ...
              {requests.data.map((r) => (
                <Fragment key={r.id}>
                  <tr className="transition-colors hover:bg-gray-50/50">
                    <td className="px-4 py-3 text-gray-700">{r.service?.title || '-'}</td>
